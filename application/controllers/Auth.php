...
class Auth extends CI_Controller
{
  ...
  private function _login()
  {
    ...
    if ($user) {
      ...
      if ($user['is_active'] == 1) {
        ...
        $this->session->set_flashdata('pesan', '<div class="alert alert-danger" role="alert">Email belum diaktivasi!</div>');
        redirect('auth');
      }
    } else {
  ...
  private function _sendEmail($token, $type)
  {
    $config = [
      ...
      'smtp_user' => 'udinuscatur@gmail.com',
      'smtp_pass' => 'udinuscatur123',
      'smtp_port' => 465,
      'mailtype' => 'html',
      'charset' => 'utf-8',
      'newline' => "\r\n"
    ...
    $this->email->from('udinuscatur@gmail.com', 'Admin Catur Udinus');
    $this->email->to($this->input->post('email'));

    if ($type == 'verify') {
      $this->email->subject('Verifikasi Akun Catur Udinus');
      $this->email->message('Klik link untuk verifikasi akun Catur Udinus
  ...
  public function lupaPassword()
  {
    ...
    if ($this->form_validation->run() == false) {
      ...
      if ($user) {
        ...
        $this->session->set_flashdata('pesan', '<div class="alert alert-success" role="alert">Silahkan check email anda untuk reset password!</div>');
        redirect('auth/lupapassword');
      } else {
        $this->session->set_flashdata('pesan', '<div class="alert alert-danger" role="alert">Email tidak teregistrasi atau telah aktif!</div>');
        redirect('auth/lupapassword');
      }
    }
  }
  ...
  public function gantiPassword()
  {
    ...
    if ($this->form_validation->run() == false) {
      ...
      $this->session->set_flashdata('pesan', '<div class="alert alert-success" role="alert">Password telah diganti! Silahkan login.</div>');
      redirect('auth');
    }
  }
}
